...
public class CardTableLayout : LayoutGroup
{
    ...
    private void UpdateLayout()
    {
        ...
        // Пересчёт может случиться раньше, чем у самой зоны посчитан её rect —
        // например, когда карта уходит со стола в руку и rebuild руки тянет за
        // собой пересборку соседей. При нулевой ширине startX уходит в большой
        // минус, а step становится отрицательным: вся зона целиком уезжает за
        // экран (масок в сцене нет, так что карты не обрезаются, а именно
        // улетают). Возвращаются они лишь при следующем корректном пересчёте —
        // это и выглядело как «принцесса забрала весь стак».
        //
        // Ничего не двигаем: система вёрстки позовёт нас снова, когда ширина
        // станет настоящей, и карты останутся на прежних местах вместо прыжка
        // в никуда.

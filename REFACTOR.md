...
### 1. Принцесса забирает весь стак карт

Если на столе три Купца, взятие «мужчины» по клику забирает все три.

**Сервер проверен и корректен.** `CardStealEffect.ApplyEffect` берёт ровно одну карту: `cards.First(c => c.Id == pick.CardId)`, затем `board.RemoveCardFromBoard(card)`. `Card` **не** переопределяет `Equals`/`GetHashCode`, а `RemoveCardFromBoard` — это `List.Remove`, то есть сравнение по ссылке и удаление одного элемента. Значит состояние на сервере не портится, и искать надо на клиенте, в отрисовке.

Две зацепки:
- `CardTableLayout.UpdateLayout` группирует карты в стопку **по тексту имени карты** (`cardView._cardNameText.text`), а не по id. Все карты группы — отдельные `CardView`, смещённые на `stackStep`.
- `ClientGameView.HandleCardDrawn` кладёт карту в руку, но **не убирает её из своего зеркала `Board`**. Рассинхрон клиентского состояния, независимо от того, он ли причина этого бага.

### 2. Меню по ESC (выйти из комнаты, выйти из игры, сдаться)

«Выйти из комнаты» **наполовину готово**: `LeaveRoomIntent` объявлен, сервер его обрабатывает (`GameConnectionManager.OnLeaveRoom` → `ReleaseConnection`), но **ни один клиент его не отправляет**. Нужна только кнопка.

«Сдаться» — новой серверной логики нет; ближайший аналог — `Room.AbortGame`, но сдача одного игрока это не то же самое, что конец комнаты. Основная стоимость всей задачи — работа в сцене (панель и кнопки).

### 3. Не видно, чей сейчас ход

**Писать код не нужно.** Всё уже есть: `TextUIManager.UpdateCurrentPlayerText()` пишет «Текущий игрок: {имя}», поле `CurrentPlayerText` в сцене привязано, объект активен (`m_IsActive: 1`), и `UnGameManager.HandleTurnAdvanced` вызывает обновление на каждом ходу.

Значит дело в видимости. Проверять в первую очередь, не лежит ли `CurrentPlayerText` на канвасе, который `LobbyManager._hideOnGameStart` гасит при старте игры, — а также размер, положение и порядок отрисовки.

### 4. При отключении игрока (3+ за столом) — карты в колоду и продолжать

Это ровно та альтернатива, которую Фаза 6.1 рассмотрела и отклонила («продолжать без игрока» против «завершить комнату»). Точка входа одна и уже помечена в коде: `Room.OnSeatVacated`.

Цена не изменилась: надо уметь выкидывать игрока из порядка ходов посреди партии — это правки в `Assets/Libreries` (`GameState`, `GameManager`) и ревизия эффектов на предмет предположений о составе. Раскладывание карт обратно в колоду — наименьшая часть работы.

### 5. Меню правил недружелюбное

UX-работа, блокирующих проблем в коде нет. Смежная известная бородавка: `Rule.Effects` — свойство, которое **на каждом обращении создаёт новый список новых экземпляров эффектов**, поэтому применённый сервером эффект никогда не тот же объект, что нажал клиент. Сегодня работает, потому что эффекты не хранят состояние.

### 6 и 7. Анимации: день/ночь не успевает; непонятная пауза при выборе (Дракон)

**Это одна задача, а не две.** Общий корень: **на клиенте нет очереди событий.** `ClientGameView` применяет каждый `DomainEvent` немедленно, а сервер шлёт их на полной скорости. Анимация не успевает доиграть до следующего события — отсюда и карты, раздающиеся поверх недоигранного помещения дня/ночи.

`AnimationManager.WaitForAllAnimations()` **существует, но в клиентском пути отрисовки его никто не вызывает.**

Это тот самый риск, который [Фаза 3](#phase-3--server-authoritative-engine) явно предвидела и предлагала закрыть вариантом (b) — «клиент буферизует события в очередь анимаций и проигрывает их последовательно». Реализовано не было.

Вторая половина — обратная связь во время чужого решения: индикатора «ждём игрока X» нет. `DecisionRequestedEvent` несёт `PlayerId` именно для этого, а `ClientGameView.OnDecisionResolved` не имеет **ни одного подписчика**.

Разумный порядок работ: сначала очередь событий, потом индикатор ожидания поверх неё.

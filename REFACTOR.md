...
- **`EnchantedForestCard` is still commented out** of the deck ([GameBuilder.cs:42](Assets/Libraries/ScaryTales/GameBuilder.cs#L42)). The effect was rewritten in Phase 4.2 and is correct for any player count — uncommenting is the whole task.
  ...
  **Проверено по коду 2026-08-31** (до этого утверждение «работает, надо только раскомментировать» кочевало из плана без перепроверки). По сути верно: цикл опроса игроков выполняется синхронно до первого `await`, так что удаление игрока посреди перебора невозможно; день, добор, пустая колода и пустые руки разобраны. Но пункт 4 (партия продолжается без вышедшего) пробил в эффекте дыру: снимок рук делается ДО `Task.WhenAll`, а игрок, вышедший во время опроса, получает автоответ, и его карты к тому моменту уже вернулись в колоду — сброс такой карты положил бы **один экземпляр сразу в две зоны**. Закрыто проверкой `player.HasCard(card)` перед сбросом.
...
### 14. Волшебник: попытка «починить» зеркало и откат — ✅ разобрано 2026-08-31

**Итог: правка отменена, вернулось прежнее поведение.** Ниже — почему, потому что ошибка тут в рассуждении, а не в коде.

**Волшебник карту в руку не БЕРЁТ.** Он раскрывает её и тут же разыгрывает; рука в этом коде — технический перевалочный пункт, нужный лишь потому, что `PlayCard` требует карту у игрока. Отправив её туда *через событие* ради стройности зеркала, я добавил в игру **визуальный шаг, которого в ней нет**, — и получил на одну карту **две анимации подряд**: «из колоды в руку» (1 с, очередь её не ждёт — так задумано, иначе раздача пяти карт превращается в пять последовательных полётов) и сразу следом «из руки на стол» (0.5 с). Обе живут одновременно на одном трансформе, и родителя карте назначает та, что финиширует **последней**, — то есть более длинный полёт в руку. Карта улетала на стол и через полсекунды возвращалась в руку. Эффект при этом срабатывал: сервер-то всё сделал правильно.

Дальше я дважды чинил последствия вместо того, чтобы усомниться в предпосылке: сперва проверкой после `await` (не в руке — не назначаем родителя), потом `transform.DOKill()`, потому что **DOTween не убивает предыдущий твин сам**. Два `DOMove` на одном трансформе просто тянут его в разные стороны. Стол успевал забрать карту и разложить, а недоигранный полёт в руку ещё полсекунды волок её прочь от разложенного места — карта зависала где-то между рукой и столом, и понять её статус было невозможно. Ни то, ни другое случая не закрыло: карта всё равно зависала между рукой и столом, а её положение приходилось ждать до следующей перерисовки. Решающим доводом стало замечание владельца — «раньше анимация Волшебника меня устраивала». Так и есть: расхождение зеркала было теоретическим (карту клиент всё равно видит следующим же событием «на стол»), а регрессия — вполне зримой. `WizardEffect` возвращён к прежнему виду.

**Обе заплатки оставлены**, потому что полезны сами по себе: `DOKill` перед новым полётом защищает любой случай, где карту двигают дважды подряд, а проверка позиции после `await` не даёт назначить устаревшего родителя. Просто теперь они не прикрывают собой лишнюю анимацию.

**Урок:** прежде чем чинить последствия своей правки во второй раз, стоит спросить, нужна ли была правка. «Раньше работало» от человека, который смотрит на экран, — это данные, а не впечатление.

**Ещё одна дыра в лесу: при пустой колоде ночная половина эффекта не срабатывала вовсе.** В начале стояли два досрочных выхода по `CardsRemaining == 0`, и второй убивал ветку сброса — хотя ночью колода не нужна, игроки сбрасывают из руки. Обнаруживалось это на исходе партии, когда колода как раз и пустеет, то есть ровно тогда, когда сброс всей руки решает исход. Взятие теперь делается «по возможности» (`DrawCard` сам сообщает о пустой колоде), а от колоды зависит только дневная ветка.

**Лес сбрасывает сразу, а не после всех.** Было `Task.WhenAll` на все ответы и общий цикл сброса после — за столом это выглядело мёртвой паузой: выбрал, и ничего не происходит, а кого ждут, непонятно. Теперь каждый игрок сбрасывает свою карту, как только ответил (`AskAndDiscard`). Опросы по-прежнему уходят одновременно: метод доходит до первого `await` синхронно, так что цикл успевает разослать все запросы, прежде чем начнёт ждать. Цена — ноль, структура даже проще прежней.

**Подсветка ожидания стала множественной.** `TextUIManager._deciding` был одним игроком, поэтому при опросе всех подсвечивался последний спрошенный, а остальные выглядели так, будто от них ничего не ждут. Теперь это множество. `DecisionResolvedEvent` несёт только номер запроса, так что `UnGameManager` держит карту «номер запроса → игрок» и снимает подсветку с того, кто действительно ответил.

### 13. «Волшебник улетел в слот дня/ночи, купцов стало два» — ✅ разобрано 2026-08-31

Симптом выглядел мистикой: разыгранный Волшебник оказался не на общем столе, а в слоте времени суток, а следом на столе задвоился Купец.

**Багом в коде это не было.** Id карты назначается **позиционно**: `Deck` и клиентский каталог идут по одному списку `GameBuilder.MakeCardTemplates()` и раздают номера подряд. «Зачарованный лес» стоит в этом списке **пятым**, поэтому его включение сдвинуло id всех последующих карт на +2. Выделенный сервер был запущен до правки, клиент — после; сервер слал «карта №45 в слот времени суток», клиент искал №45 в своём сдвинутом каталоге и честно рисовал **другую** карту.

**Настоящая проблема — что расхождение ничем не диагностировалось.** Каталог карт есть общий контракт двух процессов, и он был не версионирован: рассинхрон не падает, не логируется, а просто показывает не ту игру. Это стоило владельцу вечера и повторялось бы при каждом обновлении сервера.

Закрыто: `GameBuilder.CardCatalogVersion()` считает отпечаток по именам и количествам в порядке шаблонов, он едет в `GameStartedEvent.CardCatalogVersion`, а клиент сверяет его со своим до всего остального. При расхождении — ошибка в лог с обоими числами и **сообщение игроку на экране результата**: одного лога мало, потому что такая партия выглядит почти нормальной, карты просто «странно себя ведут».

**Побочно тронут «Волшебник» — и это оказалось ошибкой, откатано.** Он кладёт раскрытую карту в руку напрямую (`player.AddCardToHand`), минуя `GameManager`, так что события «карта попала в руку» не возникает. Расхождение зеркала настоящее, и я перевёл это на `PutCardInPlayerHand`. Разбор в пункте 14 — почему пришлось вернуть как было.

**Урок:** позиционные id — это контракт, а контракт между двумя процессами обязан иметь версию. Иначе первым сломается не то, что менялось, а всё, что стоит в списке после него.
